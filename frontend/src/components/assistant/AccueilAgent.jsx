...
function AccueilAgent() {
  return (
    ...
    >
      <Helmet>
        <title>
          Accueil Achiri | Assistant IA, accessibilité, rooms, sécurité
        </title>
        <meta
          name="description"
          content="Bienvenue sur Achiri : assistant IA, rooms vidéo/3D, accessibilité avancée, wallet, urgence, sécurité, mobile/web."
        />
      ...
      <header className="accueil-header">
        <h1>
          ...
          Bienvenue sur Achiri
        </h1>
        <p>
          Votre assistant intelligent, inclusif et sécurisé pour explorer,
          apprendre, communiquer et s'entraider.
        </p>
      </header>
      <section
        className="accueil-features"
      ...
      >
        {features.map((f, idx) => (
          <div
            ...
            aria-label={f.title}
          >
            <span className="feature-icon" role="img" aria-label={f.title}>
              {f.icon}
            </span>
            <h2>{f.title}</h2>
            <p>{f.desc}</p>
      ...
      <nav className="accueil-quicklinks" aria-label="Navigation rapide">
        <a
          ...
          aria-label="Salles vidéo"
        >
          🎥 Salles vidéo
        </a>
        <a
          href="/VirtualClassroomPage"
          className="quicklink-btn"
          aria-label="Classe virtuelle 3D"
        >
          🕶️ Classe 3D
        </a>
        <a href="/WalletPage" className="quicklink-btn" aria-label="Wallet">
          💰 Wallet
        </a>
        <a
          href="/AccessibilityPage"
          className="quicklink-btn"
