...
/**
 * Composant SFU (Selective Forwarding Unit) moderne, accessible, logique claire.
 * Gère la connexion, la production et la consommation de médias pour admin et utilisateurs.
 */
function Sfu(props) {
  ...
  const connectSendTransport = async () => {
    ...
    // Gestion des events trackended/transportclose si besoin
  };
  ...
  return (
    <div className="media-area" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div className="main-media" style={{ display: "flex", flexDirection: "row", gap: 24, alignItems: "flex-start" }}>
        ...
        <div className="my-media" style={{ marginLeft: 16 }}>
          <video
            ref={my_media}
            autoPlay
            muted
      ...
      <div className="secondary-media" style={{ display: "flex", flexWrap: "wrap", gap: 18 }}>
        {Object.entries(videoConsumers).map(([socketId, videoConsumer]) => (
          <div key={socketId} className="their-media" style={{ minWidth: 220, maxWidth: 380 }}>
            <MediaRcv
              video={videoConsumer}
