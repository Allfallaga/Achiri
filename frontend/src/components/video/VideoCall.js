...
function VideoCall({
  ...
  useEffect(() => {
    const currentLocalVideo = localVideoRef.current; // Copie pour le cleanup
    const currentRemoteVideo = remoteVideoRef.current; // Copie pour le cleanup
    return () => {
      if (currentLocalVideo) currentLocalVideo.srcObject = null; // Utilisation de la copie
      if (currentRemoteVideo) currentRemoteVideo.srcObject = null; // Utilisation de la copie
    };
  }, []);
  ...
  const toggleVideo = () => {
    if (localVideoRef.current?.srcObject) {
      ...
      if (tracks[0]) {
        ...
        if (ttsEnabled && speak)
          speak(tracks[0].enabled ? "Caméra activée" : "Caméra désactivée");
      }
    }
  };
  const toggleAudio = () => {
    if (localVideoRef.current?.srcObject) {
      ...
      if (tracks[0]) {
        ...
        if (ttsEnabled && speak)
          speak(tracks[0].enabled ? "Micro activé" : "Micro coupé");
      }
    }
  };
  ...
  return (
    ...
    >
      <Helmet>
        ...
        <meta
          name="description"
          content="Appel vidéo sécurisé Achiri : flux local/dist, avatars, accessibilité, mobile/web."
        />
      </Helmet>
      {/* Remote video */}
      <video
