...
/**
 * HealthMonitor.js (mobile) – Achiri
...
const HealthMonitor = ({ userId = "demo-user" }) => {
  ...
  if (loading) {
    return (
      <View style={styles.center} accessible accessibilityLabel="Chargement des données santé">
        <ActivityIndicator size="large" accessibilityLabel="Chargement des données santé..." color="#1976d2" />
      </View>
    );
  }
      ...
      {health ? (
        <View style={styles.dataBox} accessible accessibilityLabel="Données vitales actuelles">
          <Text style={styles.label}>
            Fréquence cardiaque : <Text style={styles.value}>{health.heartRate ?? "?"} bpm</Text>
          </Text>
          <Text style={styles.label}>
            Stress : <Text style={styles.value}>{health.stressLevel ?? "?"}</Text>
          </Text>
          <Text style={styles.label}>
            Sommeil : <Text style={styles.value}>{health.sleepHours ?? "?"} h</Text>
          </Text>
          <Text style={styles.label}>
            Dernière mesure : <Text style={styles.value}>{health.timestamp ? new Date(health.timestamp).toLocaleString() : "?"}</Text>
          </Text>
        </View>
      ) : (
        <Text style={styles.error} accessibilityLiveRegion="polite">
          Aucune donnée santé disponible.
        </Text>
      )}
        ...
        {history.length === 0 ? (
          ...
          history.map((h, i) => (
            <Text key={i} style={styles.historyItem}>
              ...
              Stress: {h.stressLevel ?? "?"}, Sommeil: {h.sleepHours ?? "?"}h
            </Text>
          ))
        )}
      </View>
