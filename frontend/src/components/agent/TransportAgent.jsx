...
const TransportAgent = ({
  ...
  const handleSend = async (msg) => {
    ...
    setTimeout(() => {
      ...
      if (/itinéraire|trajet|route/i.test(msg))
        response =
          "Pour un itinéraire accessible, utilisez la carte Achiri ou l’appli de votre ville : sélectionnez 'accessibilité' dans les filtres.";
      if (/handicap|adapté|fauteuil|mobilité/i.test(msg))
        response =
          "Transports adaptés : bus PMR, taxis adaptés, trains avec assistance, VTC accessibles. Renseignez-vous auprès des opérateurs locaux.";
      if (/horaire|bus|train/i.test(msg))
        response =
          "Consultez les horaires en temps réel sur le site de la SNCF, RATP ou via l’application Achiri.";
      if (/sécur/i.test(msg))
        response =
          "Voyagez en sécurité : informez un proche, privilégiez les lieux accessibles, gardez votre téléphone chargé.";
  ...
  return (
    ...
    >
      ...
      >
        <span role="img" aria-label="assistant transport">
          🚌
        </span>
        Agent Transport IA
      </h2>
