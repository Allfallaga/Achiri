    ...
    >
      ...
      <form autoComplete="off" onSubmit={handleAddCustom}>
        <div style={{ marginBottom: 18 }}>
          <span style={{ fontWeight: 600, marginBottom: 6, display: "block" }}>Sélectionnez vos critères :</span>
        ...
        <div style={{ marginBottom: 18 }}>
          <label htmlFor="custom-criteria" style={{ fontWeight: 600, display: "block", marginBottom: 6 }}>
        ...
        {selected.filter(c => !defaultCriteria.includes(c)).length > 0 && (
          <div style={{ marginBottom: 18 }}>
            <span style={{ fontWeight: 600, marginBottom: 6, display: "block" }}>Critères personnalisés :</span>
            <ul style={{ paddingLeft: 18, margin: 0, fontSize: 15 }}>
              {selected.filter(c => !defaultCriteria.includes(c)).map((crit, i) => (
      ...
      <div style={{ marginTop: 24, fontSize: 14, color: "#555", background: "#f0f4f8", borderRadius: 8, padding: "0.7em 1em" }}>
        <b>À propos :</b> Personnalisez vos critères d’urgence pour une alerte plus efficace et adaptée à votre situation. <a href="https://www.service-public.fr/particuliers/vosdroits/F21358" target="_blank" rel="noopener noreferrer" style={{ color: "#1976d2", textDecoration: "underline" }}>En savoir plus</a>
      </div>
      <footer
        style={{
          marginTop: 18,
