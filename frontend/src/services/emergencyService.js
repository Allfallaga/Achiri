...
export async function triggerEmergencyMock({ type, message }) {
  ...
  export async function getEmergencyLogsMock() {
    return new Promise((resolve) =>
      setTimeout(() => {
        resolve([
          { id: 1, type: "medical", message: "Urgence médicale déclenchée", date: "2024-05-01T10:00:00Z", action: "trigger" },
          { id: 2, type: "security", message: "Contact police ajouté", date: "2024-05-01T11:00:00Z", action: "addContact" },
        ]);
      }, 400)
    );
  }
   ...
   */
  export async function sendEmergencyNotificationMock({ type, message, contacts = [] }) {
    return new Promise((resolve) =>
      setTimeout(() => {
        resolve({
          success: true,
