...
test("La page Dashboard s'affiche par défaut", () => {
  ...
  expect(
    screen.getByText(/Accessibilité, santé, domotique, éducation & influence/i),
  ).toBeInTheDocument();
});
...
test("Le main content est accessible", () => {
  ...
  expect(main).toHaveAttribute(
    "aria-label",
    expect.stringMatching(/contenu principal/i),
  );
});
...
test("Header et footer ont des aria-labels", () => {
  ...
  expect(header).toHaveAttribute(
    "aria-label",
    expect.stringMatching(/en-tête/i),
  );
  expect(footer).toHaveAttribute(
    "aria-label",
    expect.stringMatching(/pied de page/i),
  );
});
