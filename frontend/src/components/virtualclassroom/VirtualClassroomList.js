 ...
 * - Fonctionnalités : liste des classes, accès rapide, participants, feedback utilisateur, dark mode, recherche, filtres.
 * - Prêt pour extensions futures (favoris, analytics, overlay, badges, IA…).
 *
 * Props :
 *   userId: string (identifiant utilisateur affiché)
 *   onSelect: function (callback lors de la sélection d’une classe)
 */
...
const mockClassrooms = [
  ...
  },
  {
    ...
    participants: 8,
    accessibility: ["Sous-titres", "Traduction"]
  },
];
...
export default function VirtualClassroomList({ userId, onSelect }) {
  ...
  // Filtrage simple par nom ou description
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <FaBookOpen /> {filtered.length} classes &nbsp;|&nbsp; Utilisateur : <strong>{userId}</strong>
      </div>
      <input
        type="search"
        value={search}
      ...
      <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
        {filtered.map((c) => (
          ...
          >
            <div>
              <div style={{ fontWeight: 600, fontSize: "1.1em", color: darkMode ? "#ffe082" : "#1976d2" }}>{c.name}</div>
              <div style={{ fontSize: "0.97em", color: darkMode ? "#bbb" : "#555", marginTop: 2 }}>{c.description}</div>
              <div style={{
                fontSize: "0.92em",
                color: "#43a047",
                marginTop: 4,
                ...
                {c.accessibility && c.accessibility.map((a, i) => (
